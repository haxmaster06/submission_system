          ...
          <div className="flex gap-2">
            {(['all', 'under', 'over', 'exact'] as const).map(s => (
              <button
                ...
                className={`px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${filterStatus === s
            ...
            {filtered.map(s => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${s.status === 'over' ? 'bg-rose-50 text-rose-600' : s.status === 'exact' ? 'bg-sky-50 text-sky-600' : 'bg-emerald-50 text-emerald-600'
                    }`}>
          ...
          {selectedSubmission && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                className="relative w-full max-w-7xl bg-slate-50 rounded-[40px] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col"
              >
